...
'-------------------/
	...
	* Added the ability to toggle dithering on/off
	...
	Version by makuznetsov73
	+ Added edge detection based on Sobel operator which you can turn on/off. Inspired by Acerola's video https://www.youtube.com/watch?v=gg40RWiaHRY
	+ Added new Font color mode. In this mode background of ASCII characters would still have a little color
		making the image brighter (and nicer to look at in my opinion)
*/
...
uniform bool Ascii_use_edges <
	ui_label = "Draw edges";
ui_tooltip = "0 = Regular style, no edges, 1 = Draw edges using Sobel operator";
ui_category = "Font style";
> = 0;

uniform int Sobel_threshold_percent < __UNIFORM_SLIDER_INT1
										  ui_min = 0;
ui_max = 100;
ui_label = "Sobel filter threshold value";
ui_tooltip = "Sobel filter threshold value in percent for deciding if it is a edge or not";
ui_category = "Font style";
> = 10;
...
ui_tooltip = "0 = Foreground color on background color, 1 = Colorized grayscale, 2 = Full color, 3 = Background painted";
...
#define sampler ReShade::BackBuffer
#define asciiSampler ReShade::BackBuffer

static const float PI = 3.14159265f;

texture2D AFX_AsciiSobelTex
{
	Width = BUFFER_WIDTH;
...
int SobelTypeLetter(float2 block_size, float2 cursor_position)
{
	// type 0 - normal letter
	// type 1 - \
	// type 2 - -
	// type 3 - /
	// type 4 - |
	...
	for (int i = 0; i < block_size.x; i++)
	{
		...
		{
			float3 sobel_value = tex2D(Sobel, cursor_position + float2(i, j) * BUFFER_PIXEL_SIZE).rgb;
			if (sobel_value.x == 1.0)
			{
				counters[1]++;
			}
			else if (sobel_value.x == 2.0)
			{
				counters[2]++;
			}
			else if (sobel_value.x == 3.0)
			{
				counters[3]++;
			}
			else if (sobel_value.x == 4.0)
			{
				counters[4]++;
			}
			else
	...
	if (max > total * Sobel_threshold_percent / 100)
	{
		return max_pos;
	}
	return 0;
...
float3 AsciiPass(float2 tex)
{
	...
	if (Ascii_use_edges)
	{
		sobel_type = SobelTypeLetter(Ascii_block, cursor_position);
	}
	...
	*/
	...
	else
	{
		if (Ascii_font_color_mode == 3)
		{
			if (sobel_type == 0)
			{
				color = (character) ? character * color : color * 0.5;
			}
			else
			{
				color = (character) ? character * color : color * 0.35;
			}
		}
		else if (Ascii_font_color_mode == 2)
		{
...
float SobelType(float2 tex)
{
	float3 sobelX = tex2D(sampler, tex).rgb * -1 +
					tex2D(sampler, tex + float2(2.0, 0.0) * BUFFER_PIXEL_SIZE).rgb * 1 +
					tex2D(sampler, tex + float2(0.0, 1.0) * BUFFER_PIXEL_SIZE).rgb * -2 +
					tex2D(sampler, tex + float2(2.0, 1.0) * BUFFER_PIXEL_SIZE).rgb * 2 +
					tex2D(sampler, tex + float2(0.0, 2.0) * BUFFER_PIXEL_SIZE).rgb * -1 +
					tex2D(sampler, tex + float2(2.0, 2.0) * BUFFER_PIXEL_SIZE).rgb * 1;

	float3 sobelY = tex2D(sampler, tex).rgb * -1 +
					tex2D(sampler, tex + float2(1.0, 0.0) * BUFFER_PIXEL_SIZE).rgb * -2 +
					tex2D(sampler, tex + float2(2.0, 0.0) * BUFFER_PIXEL_SIZE).rgb * -1 +
					tex2D(sampler, tex + float2(0.0, 2.0) * BUFFER_PIXEL_SIZE).rgb * 1 +
					tex2D(sampler, tex + float2(1.0, 2.0) * BUFFER_PIXEL_SIZE).rgb * 2 +
					tex2D(sampler, tex + float2(2.0, 2.0) * BUFFER_PIXEL_SIZE).rgb * 1;

	float3 color = sqrt(sobelX * sobelX + sobelY * sobelY);

	float luma = dot(color, float3(0.2126, 0.7152, 0.0722));
	if (luma > 0.5)
	{
		float degrees = atan(dot(sobelY, float3(0.2126, 0.7152, 0.0722)) / dot(sobelX, float3(0.2126, 0.7152, 0.0722)));
		if (degrees > PI * 0.375)
		{
			return 4.0;
		}
		else if (degrees > PI * 0.125)
		{
			return 3.0;
		}
		else if (degrees > PI * -0.125)
...
float PS_Sobel(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
	if (Ascii_use_edges)
	{
		return SobelType(texcoord);
	}
	else
	{
		return 0.0;
